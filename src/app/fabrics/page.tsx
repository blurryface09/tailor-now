    ...
    <div className="min-h-screen bg-[#09090B]">
      ...
      <div className="relative max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 bg-amber-400/10 border border-amber-400/20 text-amber-400 text-xs font-bold px-4 py-1.5 rounded-full mb-4">
            <Layers size={13} /> TailorNow Sourced
          </div>
          <h1 className="text-3xl md:text-4xl font-black text-white mb-3">
            Our Fabric <span className="bg-gradient-to-r from-amber-400 to-orange-400 bg-clip-text text-transparent">Catalogue</span>
          </h1>
        ...
        <div className="relative mb-5 max-w-md mx-auto">
          ...
          <input
            className="w-full pl-10 pr-4 py-3 rounded-2xl bg-white/[0.05] border border-white/[0.09] text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:border-amber-500/40 transition-all"
        ...
        <div className="flex gap-2 flex-wrap justify-center mb-8">
          {TYPE_CHIPS.map(([val, label]) => (
            <button key={val} onClick={() => setFilterType(val)}
              className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all border ${filterType === val ? 'bg-amber-400 text-black border-amber-400' : 'bg-white/[0.04] border-white/[0.08] text-zinc-400 hover:text-white'}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-zinc-500">
            ...
            <p className="font-medium text-white mb-1">No fabrics found</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {displayed.map((fabric, idx) => (
              <button key={fabric.id} onClick={() => setSelected(selected?.id === fabric.id ? null : fabric)}
                className={`group text-left bg-white/[0.04] border rounded-2xl overflow-hidden transition-all duration-300 ${selected?.id === fabric.id ? 'border-amber-500/50 shadow-lg shadow-amber-500/10 scale-[1.02]' : 'border-white/[0.07] hover:border-amber-500/20'}`}
                ...
                {/* Info */}
                <div className="p-3">
                  <p className="font-bold text-white text-sm leading-snug mb-0.5">{fabric.name}</p>
