...
const PARTICLES = Array.from({ length: 55 }, (_, i) => ({
  id: i,
  left: `${((i * 19.7 + 7.3) % 100).toFixed(1)}%`,
  top: `${((i * 31.1 + 13.7) % 100).toFixed(1)}%`,
  size: `${(((i * 7 + 1) % 3) + 1).toFixed(1)}px`,
  dur: `${(((i * 11) % 12) + 8).toFixed(1)}s`,
  delay: `${(-((i * 5.7) % 20)).toFixed(1)}s`,
  opacity: ((((i * 13) % 5) + 1) * 0.04 + 0.04).toFixed(2),
  color: i % 5 === 0 ? '#F59E0B' : '#7C3AED',
}))
  ...
  return (
    <div className="min-h-screen bg-[#09090B] text-white overflow-x-hidden selection:bg-violet-500/30">
      ...
      <nav className="fixed top-0 z-50 w-full px-4 pt-3">
        <div className="max-w-7xl mx-auto bg-black/50 backdrop-blur-2xl border border-white/[0.08] rounded-2xl px-5 h-14 flex items-center justify-between shadow-2xl shadow-black/60">
          <Link href="/"><Logo size="sm" variant="full" dark /></Link>
          <div className="hidden md:flex items-center gap-6 text-sm text-zinc-500">
            <Link href="/browse" className="hover:text-white transition-colors duration-200">Find Creatives</Link>
            <Link href="/signup?as=tailor" className="hover:text-white transition-colors duration-200">Join as Creative</Link>
            <a href="#how-it-works" className="hover:text-white transition-colors duration-200">How it works</a>
            <Link href="/hall-of-fame" className="hover:text-amber-400 transition-colors duration-200 font-medium">🏆 Hall of Fame</Link>
          </div>
          <div className="flex items-center gap-2.5">
            <Link href="/login" className="hidden md:block text-sm font-medium text-zinc-500 hover:text-white transition-colors px-3 py-2 rounded-xl hover:bg-white/[0.06]">
              Log in
            </Link>
      ...
      >
        {/* Animated ambient orbs — move with mouse at different speeds */}
        <div className="absolute top-0 left-0 w-[700px] h-[700px] rounded-full pointer-events-none"
          style={{ background: 'radial-gradient(circle, rgba(109,40,217,0.16) 0%, transparent 65%)', transform: `translate(${mouse.x * -30}px, ${mouse.y * -30}px)`, transition: 'transform 0.9s ease-out' }} />
        <div className="absolute bottom-0 right-0 w-[600px] h-[600px] rounded-full pointer-events-none"
          style={{ background: 'radial-gradient(circle, rgba(217,119,6,0.1) 0%, transparent 65%)', transform: `translate(${mouse.x * 25}px, ${mouse.y * 25}px)`, transition: 'transform 0.9s ease-out' }} />
        ...
        <div className="relative w-full max-w-7xl mx-auto px-5 grid lg:grid-cols-2 gap-16 items-center py-24 lg:py-0 min-h-[calc(100svh-5rem)]">
          ...
          <div className="order-2 lg:order-1 flex flex-col justify-center">
            <div className="inline-flex items-center gap-2.5 bg-white/[0.06] border border-white/[0.1] rounded-full px-4 py-2 text-xs w-fit backdrop-blur-sm mb-8 fade-up">
              <span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" />
              <span className="text-zinc-400">Nigeria&apos;s #1 fashion marketplace</span>
            ...
            <div className="flex flex-wrap gap-3.5 mb-12 fade-up-3">
              <Link href="/browse"
                className="group flex items-center gap-2.5 bg-amber-400 hover:bg-amber-300 text-black font-black text-base px-8 py-4 rounded-2xl transition-all duration-200 hover:scale-[1.04] active:scale-[0.97] shadow-2xl shadow-amber-500/30">
                Find a Creative
                <ArrowRight size={18} className="group-hover:translate-x-1.5 transition-transform duration-200" />
              </Link>
              <Link href="/signup?as=tailor"
                className="flex items-center gap-2.5 bg-white/[0.07] hover:bg-white/[0.12] border border-white/[0.12] hover:border-violet-500/40 text-white font-bold text-base px-8 py-4 rounded-2xl transition-all duration-200 backdrop-blur-sm">
                Join as Creative
              </Link>
            </div>

            {/* Animated stats */}
            <div className="grid grid-cols-4 gap-4 border-t border-white/[0.08] pt-8 fade-up-4">
              {[
                ...
                <div key={s.label}>
                  <div className="text-xl lg:text-2xl font-black text-amber-400 tabular-nums">
          ...
          <div className="order-1 lg:order-2 flex justify-center lg:justify-end">
            <div className="relative w-80 h-[480px] lg:w-[420px] lg:h-[560px]">
              ...
              {/* Layer 3 — deepest, moves most */}
              <div className="absolute bottom-0 left-0 w-[210px] h-[300px] lg:w-[255px] lg:h-[365px] rounded-3xl overflow-hidden border border-white/[0.1] shadow-2xl shadow-black/80"
              ...
              {/* Layer 2 — mid depth */}
              <div className="absolute top-8 left-14 w-[185px] h-[265px] lg:w-[220px] lg:h-[320px] rounded-3xl overflow-hidden border border-violet-500/20 shadow-2xl shadow-violet-900/50 z-[1]"
              ...
              {/* Layer 1 — front, moves least */}
              <div className="absolute top-0 right-0 w-[225px] h-[315px] lg:w-[265px] lg:h-[385px] rounded-3xl overflow-hidden border border-white/[0.1] shadow-2xl shadow-black/60 z-[2]"
              ...
              {/* Floating UI: creative card — wrapper floats, inner tracks mouse */}
              <div className="absolute bottom-10 -left-4 lg:-left-10 z-[10] float-y">
                <div className="bg-black/75 backdrop-blur-xl rounded-2xl px-4 py-3 border border-white/[0.1] shadow-2xl"
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-zinc-500">⭐ 4.9 · Lagos · Verified</p>
                    </div>
                    <CheckCircle size={14} className="text-violet-400 ml-1 flex-shrink-0" />
                  </div>
                </div>
              </div>
              ...
              <div className="absolute top-6 -left-2 lg:-left-8 z-[10] float-y" style={{ animationDelay: '1.4s' }}>
                <div className="bg-black/75 backdrop-blur-xl rounded-2xl px-3.5 py-2.5 border border-white/[0.08] shadow-xl"
                  style={{ transform: `translate(${mouse.x * -55}px, ${mouse.y * -22}px)`, transition: 'transform 1s ease-out' }}>
                  <p className="text-[10px] text-zinc-500 font-medium">📦 Order update</p>
              ...
              <div className="absolute top-1/2 -translate-y-1/2 -right-2 lg:-right-8 z-[10]">
                <div className="bg-black/75 backdrop-blur-xl rounded-2xl px-3.5 py-3 border border-amber-500/20 shadow-xl"
                  ...
                  <p className="text-[10px] font-bold text-white">Perfect fit! ✨</p>
                  <p className="text-[9px] text-zinc-600 mt-0.5">by Chisom A.</p>
                </div>
              </div>

              {/* Ambient glow behind the stack */}
              <div className="absolute inset-12 rounded-full bg-violet-600/15 blur-3xl pointer-events-none z-[-1]" />
            </div>
          </div>
        </div>
      ...
      {/* ── Marquee band ─────────────────────────────────────────────────────── */}
      <div className="relative overflow-hidden border-y border-white/[0.06] py-4 bg-white/[0.015]">
        <div className="flex marquee-track">
          {[...MARQUEE_ITEMS, ...MARQUEE_ITEMS].map((item, i) => (
            <span key={i} className="flex-shrink-0 flex items-center gap-5 px-7 text-xs font-bold uppercase tracking-widest text-zinc-600">
      ...
      <section className="py-28 max-w-7xl mx-auto px-5">
        <Reveal className="text-center mb-16">
          <p className="text-xs font-bold tracking-widest text-violet-400 uppercase mb-4">What you can order</p>
          <h2 className="text-4xl lg:text-5xl font-black">Fashion, made for you</h2>
      ...
      {/* ── How it works ─────────────────────────────────────────────────────── */}
      <section id="how-it-works" className="py-24 bg-white/[0.015] border-y border-white/[0.06]">
        <div className="max-w-7xl mx-auto px-5">
          ...
          <div className="grid md:grid-cols-4 gap-4 relative">
            <div className="absolute top-[3.2rem] left-[12%] right-[12%] h-px border-t-2 border-dashed border-white/[0.05] hidden md:block pointer-events-none" />
            {STEPS.map((step, i) => (
              <Reveal key={step.n} delay={i * 100}>
                <TiltCard strength={8} className="relative bg-white/[0.04] border border-white/[0.08] rounded-3xl p-7 group hover:bg-white/[0.07] hover:border-violet-500/20 transition-colors duration-300 cursor-default h-full">
                  <div className="absolute top-5 right-5 text-5xl font-black text-white/[0.04] select-none leading-none">{step.n}</div>
                  <div className="w-14 h-14 rounded-2xl bg-violet-500/10 border border-violet-500/20 flex items-center justify-center text-2xl mb-5 group-hover:scale-110 group-hover:bg-violet-500/20 transition-all duration-300">
                    {step.icon}
                  </div>
      ...
      <section className="py-28 max-w-7xl mx-auto px-5">
        <Reveal className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-14">
          <div>
            <p className="text-xs font-bold tracking-widest text-violet-400 uppercase mb-4">Featured creatives</p>
            <h2 className="text-4xl font-black">Meet Nigeria&apos;s<br />finest tailors</h2>
          </div>
          <Link href="/browse" className="group flex items-center gap-2 text-violet-400 hover:text-violet-300 font-bold text-sm transition-colors whitespace-nowrap">
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {CREATIVES.map((c, i) => (
            <Reveal key={c.name} delay={i * 100}>
              <TiltCard strength={8} className="group relative bg-white/[0.04] border border-white/[0.07] rounded-3xl overflow-hidden cursor-default hover:border-white/[0.14] transition-colors duration-300 h-full">
                {/* Pattern header */}
                <div className={`h-28 bg-gradient-to-br ${c.grad} relative overflow-hidden`} style={{ opacity: 0.35 }}>
                  <div className="absolute inset-0" style={{ backgroundImage: 'radial-gradient(circle at 2px 2px, white 1px, transparent 0)', backgroundSize: '20px 20px', opacity: 0.2 }} />
                </div>
                <div className="px-6 pb-6">
                  <div className="-mt-8 mb-4">
                    <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${c.grad} flex items-center justify-center text-white text-2xl font-black shadow-2xl ${c.glow} border-4 border-[#09090B]`}>
                      {c.init}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 mt-5 pt-4 border-t border-white/[0.07]">
                    <div>
                      <div className="flex items-center gap-1 text-amber-400 font-bold text-sm"><Star size={12} className="fill-amber-400" />{c.rating}</div>
                    ...
                    <Link href="/browse" className="ml-auto flex items-center gap-1.5 bg-violet-500/10 hover:bg-violet-500/20 border border-violet-500/20 text-violet-300 text-xs font-bold px-3 py-1.5 rounded-full transition-colors">
      ...
      {/* ── Features bento ───────────────────────────────────────────────────── */}
      <section className="py-24 bg-white/[0.015] border-y border-white/[0.06]">
        <div className="max-w-7xl mx-auto px-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {FEATURES.map((f, i) => (
              <Reveal key={f.title} delay={i * 70} className={i === 0 || i === 5 ? 'col-span-2 md:col-span-1' : ''}>
                <TiltCard strength={6} className="bg-white/[0.04] border border-white/[0.08] rounded-3xl p-7 group hover:bg-white/[0.07] hover:border-white/[0.14] transition-all duration-300 h-full cursor-default">
      ...
      <section className="py-20 max-w-7xl mx-auto px-5">
        <TiltCard strength={4} className="relative rounded-3xl overflow-hidden border border-white/[0.08] p-12 lg:p-16 cursor-default"
          style={{ background: 'radial-gradient(ellipse at 30% 50%, rgba(109,40,217,0.15) 0%, transparent 60%), radial-gradient(ellipse at 70% 50%, rgba(217,119,6,0.1) 0%, transparent 60%), rgba(255,255,255,0.02)' }}>
          <div className="absolute inset-0 opacity-[0.018] pointer-events-none"
            style={{ backgroundImage: 'radial-gradient(circle at 1px 1px, white 1px, transparent 0)', backgroundSize: '24px 24px' }} />
          <Reveal className="text-center mb-12">
            <h2 className="text-3xl lg:text-5xl font-black">Growing every day</h2>
            <p className="text-zinc-500 mt-3 text-sm">Real numbers. Real fashion. Real Nigeria.</p>
          </Reveal>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center relative">
            {[
              ...
              <Reveal key={s.label} delay={i * 120}>
                <div className="text-4xl lg:text-5xl font-black text-amber-400 mb-2 tabular-nums">
                  <CountUp to={s.to} suffix={s.suffix} />
                </div>
      ...
      <section className="pb-28 max-w-7xl mx-auto px-5">
        <div className="grid md:grid-cols-2 gap-5">
          ...
          <Reveal>
            <TiltCard strength={6} className="relative overflow-hidden rounded-3xl p-10 border border-violet-500/20 h-full cursor-default"
              style={{ background: 'linear-gradient(135deg, rgba(109,40,217,0.25) 0%, rgba(49,18,105,0.45) 100%)' }}>
              <div className="absolute -right-16 -bottom-16 w-72 h-72 bg-violet-500/15 rounded-full blur-3xl pointer-events-none" />
              <div className="absolute inset-0 opacity-[0.025] pointer-events-none"
                style={{ backgroundImage: 'radial-gradient(circle at 2px 2px, white 1px, transparent 0)', backgroundSize: '20px 20px' }} />
              <div className="relative">
                <div className="inline-flex items-center gap-2 bg-violet-500/20 border border-violet-500/30 rounded-full px-3.5 py-1.5 text-xs font-bold text-violet-300 mb-6">
                  <Users size={12} /> For customers
                </div>
                <h3 className="text-3xl lg:text-4xl font-black text-white mb-3 leading-tight">Find your perfect<br />creative today</h3>
                <p className="text-violet-200/55 text-sm mb-8 leading-relaxed">Browse hundreds of verified creatives across Lagos, Abuja, Port Harcourt and all of Nigeria.</p>
                <Link href="/browse"
                  className="group inline-flex items-center gap-2.5 bg-white hover:bg-violet-50 text-violet-900 font-black px-7 py-3.5 rounded-2xl transition-all hover:scale-[1.04] shadow-2xl shadow-violet-900/40">
          ...
          <Reveal delay={80}>
            <TiltCard strength={6} className="relative overflow-hidden rounded-3xl p-10 border border-amber-500/20 h-full cursor-default"
              style={{ background: 'linear-gradient(135deg, #130b00 0%, #1e1100 100%)' }}>
              ...
              <div className="relative">
                <div className="inline-flex items-center gap-2 bg-amber-500/20 border border-amber-500/30 rounded-full px-3.5 py-1.5 text-xs font-bold text-amber-300 mb-6">
                  <Zap size={12} /> For creatives
                </div>
                <h3 className="text-3xl lg:text-4xl font-black text-white mb-3 leading-tight">Turn your skill<br />into income</h3>
                <p className="text-amber-100/40 text-sm mb-8 leading-relaxed">Get verified, build your portfolio, and reach thousands of customers ready to book today.</p>
                <Link href="/signup?as=tailor"
                  className="group inline-flex items-center gap-2.5 bg-amber-400 hover:bg-amber-300 text-black font-black px-7 py-3.5 rounded-2xl transition-all hover:scale-[1.04] shadow-2xl shadow-amber-500/20">
      ...
      <footer className="border-t border-white/[0.06] py-14">
        <div className="max-w-7xl mx-auto px-5">
          ...
          <div className="border-t border-white/[0.06] pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-xs text-zinc-600">© 2026 TailorNow. All rights reserved.</p>
            <p className="text-xs text-zinc-600">Built by <span className="text-violet-400 font-semibold">Folub and Samuel Labs</span></p>
          </div>
        </div>
      </footer>
