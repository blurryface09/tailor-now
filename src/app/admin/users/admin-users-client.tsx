    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">{users.length} total accounts</p>
        </div>
        <input
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.length === 0 && (
                <tr><td colSpan={7} className="text-center py-12 text-zinc-600">No users found</td></tr>
              )}
              {filtered.map(u => {
                const roleCfg = ROLE_CONFIG[u.role as keyof typeof ROLE_CONFIG] || ROLE_CONFIG.customer
                return (
                  <tr key={u.id} className="hover:bg-white/[0.06] transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-white">{u.full_name || <span className="text-zinc-600">No name</span>}</p>
                    ...
                    <td className="px-4 py-3">
                      <div className="space-y-0.5">
                        {u.email && <p className="flex items-center gap-1 text-xs text-zinc-500"><Mail size={11} /> {u.email}</p>}
                        {u.phone && <p className="flex items-center gap-1 text-xs text-zinc-500"><Phone size={11} /> {u.phone}</p>}
                        {!u.email && !u.phone && <p className="text-xs text-zinc-600">—</p>}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      {u.city
                        ? <span className="flex items-center gap-1 text-xs text-zinc-500"><MapPin size={11} />{u.city}{u.state ? `, ${u.state}` : ''}</span>
                    ...
                    <td className="px-4 py-3 font-medium text-zinc-300">{orderCounts[u.id] || 0}</td>
                    <td className="px-4 py-3 text-xs text-zinc-600">{formatDate(u.created_at)}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        {u.role === 'tailor' && (
                          <button onClick={() => demoteUser(u.id, u.role)}
                            className="flex items-center gap-1 px-2.5 py-1 text-xs text-zinc-500 hover:text-red-600 hover:bg-red-500/10 rounded-lg border border-transparent hover:border-red-200 transition-all"
